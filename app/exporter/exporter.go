...
func (e Exporter) processImageResult(result ImageResult, ch chan<- prometheus.Metric) {
	if result.Name != "" && result.Namespace != "" {
		...
		ch <- prometheus.MustNewConstMetric(dockerHubImageStars, prometheus.GaugeValue, result.StarCount, result.Name, result.Namespace)
		ch <- prometheus.MustNewConstMetric(dockerHubImageIsAutomated, prometheus.GaugeValue, isAutomated, result.Name, result.Namespace)
		ch <- prometheus.MustNewConstMetric(dockerHubImagePullsTotal, prometheus.CounterValue, result.PullCount, result.Name, result.Namespace)
